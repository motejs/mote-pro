...
export const formatDollar = function (num, settings) {
	try {
		num = Number(num);
...
export const formatCurrency = function (c, num, settings) {
	let map = {
		USD: 'US$'
	}
	let currencyUnit;
	if (c) {
		currencyUnit = map[c.toString().toUpperCase()] || c
	}

	try {
		num = Number(num);
		return `${num >= 0 ? '' : '-'}${currencyUnit} ${currency(Math.abs(num), settings).format()}`
	} catch (err) {
		console.log(err, 'error from formatCurrency: num should be a number.');
		return num;
	}
...
export function CurrencyMap(c) {
	let map = {
		USD: 'US$'
	}
	if (c) {
		return map[c.toString().toUpperCase()] || c;
	}
	return 'US$'
}
